...
#include <omplapp/apps/MultiRigidBodyPlanning.h>
...
void benchmark0(std::string& benchmark_name, app::MultiRigidBodyPlanning& setup,
                double& runtime_limit, double& memory_limit, int& run_count)
{
    ...
    start.get()->components[0]->as<base::SE3StateSpace::StateType>()->setX(-4.96);
    start.get()->components[0]->as<base::SE3StateSpace::StateType>()->setY(-40.62);
    start.get()->components[0]->as<base::SE3StateSpace::StateType>()->setZ(70.57);
    start.get()->components[0]->as<base::SE3StateSpace::StateType>()->rotation().setIdentity();

    base::ScopedState<base::CompoundStateSpace> goal(start);
    goal.get()->components[0]->as<base::SE3StateSpace::StateType>()->setX(200.49);
    goal.get()->components[0]->as<base::SE3StateSpace::StateType>()->setY(-40.62);
    goal.get()->components[0]->as<base::SE3StateSpace::StateType>()->setZ(70.57);
    goal.get()->components[0]->as<base::SE3StateSpace::StateType>()->rotation().setIdentity();
...
void benchmark1(std::string& benchmark_name, app::MultiRigidBodyPlanning& setup,
                double& runtime_limit, double& memory_limit, int& run_count)
{
    ...
    base::ScopedState<base::CompoundStateSpace> start(setup.getSpaceInformation());
    start.get()->components[0]->as<base::SE3StateSpace::StateType>()->setX(270.);
    start.get()->components[0]->as<base::SE3StateSpace::StateType>()->setY(160.);
    start.get()->components[0]->as<base::SE3StateSpace::StateType>()->setZ(-200.);
    start.get()->components[0]->as<base::SE3StateSpace::StateType>()->rotation().setIdentity();

    base::ScopedState<base::CompoundStateSpace> goal(start);
    goal.get()->components[0]->as<base::SE3StateSpace::StateType>()->setX(270.);
    goal.get()->components[0]->as<base::SE3StateSpace::StateType>()->setY(160.);
    goal.get()->components[0]->as<base::SE3StateSpace::StateType>()->setZ(-400.);
    goal.get()->components[0]->as<base::SE3StateSpace::StateType>()->rotation().setIdentity();
    ...
    bounds.setLow(1,75.);
    bounds.setLow(2,-450.);
    setup.getStateSpace()->as<base::CompoundStateSpace>()->getSubSpace(0)->as<base::SE3StateSpace>()->setBounds(bounds);
...
{
    app::MultiRigidBodyPlanning setup(1);
